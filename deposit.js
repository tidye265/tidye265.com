// deposit.js - Fixed for Router Lifecycle (Cleanup & No Auto-Exec)
(function() {
    "use strict";

    // ── PAGE INIT FUNCTION (to be called by router) ──
    window.initDepositPage = async function() {
        console.log("⚡ [DEPOSIT JS] initDepositPage() called by router.");
        ...
        // ── ROUTER HELPERS ──
        const router = window.tidyeRouter;
        if (!router) {
            console.error("tidyeRouter not found! Please ensure router.js is loaded first.");
            return;
        }

        // ── REGISTER GLOBALS (so they get cleaned up) ──
        const globalNames = ['_depositClickHandler', 'processDeposit', 'toggleNetworkDetails', 'setAmount'];
        globalNames.forEach(name => router.registerGlobal(name));

        // ── REGISTER TIMERS & LISTENERS (will be cleaned on page destroy) ──
        // We'll register them as we create them.

        // ── HELPER FUNCTIONS (unchanged, but using router for cleanup) ──
        ...
        // ── PROCESS DEPOSIT (unchanged, but we'll register timers) ──
        ...
        function stopPolling() {
            if (successPollInterval) {
                ...
                // Remove from router's timer list (if registered)
                // Actually we rely on router to clean up; we just clear here.
                successPollInterval = null;
            }
            if (failurePollInterval) {
                clearInterval(failurePollInterval);
        ...
        async function pollForDepositSuccess(amount) {
            ...
            // Register the interval with router for cleanup
            router.registerTimer(successPollInterval);
        }
        ...
        window.processDeposit = async function() {
            ...
            try {
                ...
                if (res.ok && data.success) {
                    ...
                    // Register failure interval
                    router.registerTimer(failurePollInterval);
                } else {
        ...
        // ── TURNSTILE (unchanged, but we register the refresh interval) ──
        ...
        function initTurnstile() {
            ...
            if (!container) {
                ...
                try {
                    ...
                    console.error("Turnstile appendChild error:", e);
                }
            }
        ...
        // ── EVENT LISTENERS (attached fresh each time, but registered with router) ──
        // Remove any previous handler to avoid duplicates (router will have cleaned it, but just in case)
        ...
        router.registerListener(document, 'click', window._depositClickHandler);
        ...
        const turnstileRefreshInterval = setInterval(() => {
            if (tokenCreatedAt && (Date.now() - tokenCreatedAt > 80000)) refreshTurnstile();
        }, 10000);
        router.registerTimer(turnstileRefreshInterval);
        ...
        function setupSeeMore() {
            ...
            if (moreBtn && subHeader) {
                ...
                router.registerListener(newBtn, 'click', newBtn._listener); // but we didn't store the listener, we can register via closure
                // Simple approach: store the handler in a variable and register it.
                // We'll do it inline:
                ...
                router.registerListener(newBtn, 'click', handler);
            }
        }
        setupSeeMore();
        ...
        router.registerListener(window, 'offline', updateOnlineStatus);
        router.registerListener(window, 'online', updateOnlineStatus);
        updateOnlineStatus();
        ...
        router.setCleanup(function() {
            // Stop all polling
            stopPolling();
            // Remove any remaining listeners (router will remove registered ones, but we can clear)
            // The router already removes registered listeners, but we can also remove the offline/online listeners if not registered?
            // They are registered, so they will be removed.
            // Remove the click handler if still present (router will delete the global and remove listener)
            // We'll also ensure turnstile widget is reset
            if (typeof turnstile !== 'undefined' && turnstileWidgetId !== null) {
                turnstile.reset(turnstileWidgetId);
            }
            // Remove any leftover banners
    ...
    // ── NO AUTO-EXECUTION! ──
    // The router will call window.initDepositPage() when navigating to deposit.html.
})();
