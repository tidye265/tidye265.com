// deposit.js - SPA‑Ready Page Controller (v2 - Fixed JSON Parse Errors)
window.initDepositPage = async function() {
    ...
    // ── FIX: Check if Supabase vars exist ──
    if (!window.SUPABASE_URL || !window.SUPABASE_ANON_KEY) {
        console.error("❌ CRITICAL: window.SUPABASE_URL or ANON_KEY is undefined. Check your global variables.");
        document.getElementById('app-content').innerHTML = `<div style="color:red;padding:20px;">Supabase variables are missing. Please configure them in your layout.</div>`;
        return;
    }
    ...
    // ── FIX: TURNSTILE TOKEN HELPER (Reads from window._turnstileToken first, then sessionStorage) ──
    let turnstileToken = null;
    let tokenCreatedAt = 0;

    function getActiveTurnstileToken() {
        // 1. Check router's global variable
