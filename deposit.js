// deposit.js - SPA‑Safe Page Controller

// ── GLOBAL FLAG: Kuti tipewe kuyika event listeners kawiri ──
// Izi zimasinthidwa pa init iliyonse.
let _depositEventsAttached = false;
...
window.initDepositPage = async function() {
    ...
    // ── 5. FETCH FUNCTIONS (inside init scope) ──
    ...
    window.processDeposit = async function() {
        ...
        if (!globalUserPhone || globalUserPhone === 'Not provided' || !globalDetectedNetwork || globalDetectedNetwork === 'UNKNOWN' || !globalUserId) {
            showStatus("Phone or network not recognized.", "error");
            return;
        }
