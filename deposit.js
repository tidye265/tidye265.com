// deposit.js - SPA‑Ready Page Controller (v4 - AbortController & Lifecycle Integration)
window.initDepositPage = async function() {
    "use strict";

    console.log("⚡ [DEPOSIT JS] initDepositPage started.");

    // ── FIX: Hybrid Wait (Event + Polling) ──
        ...
        return new Promise((resolve, reject) => {
            ...
            const checkExist = setInterval(() => {
                ...
                if (attempts >= maxAttempts) {
                    clearInterval(checkExist);
                    reject(new Error(`Supabase variables are missing. Please configure them in your layout by adding <script src='supabase.js'></script> inside the <head> tag of your index.html file, **ABOVE** the router.js script.`));
                }
            }, 100);
    ...
    try {
        ...
        if (container) {
            container.innerHTML = `<div style="padding:20px; background:#fef2f2; border: 1px solid #fecaca; border-radius:12px; text-align:center; font-family:'Lexend', sans-serif;">
                <h3 style="color:#dc2626;">Supabase Variables Missing</h3>
                <p style="color:#991b1b; font-size:14px; margin-top:8px;">${e.message}</p>
                <p style="color:#6b7280; font-size:13px; margin-top:12px;">Please check the browser console for more details.</p>
            </div>`;
        }
        return;
    }
    ...
    // ⚡ UPGRADE: Added options/signal to support Router v11 AbortController
    function fetchWithRetry(url, options = {}, retries = 3, delay = 500) {
        return new Promise((resolve, reject) => {
            let attempt = 0;
            function doFetch() {
                attempt++;
                fetch(url, options) // Pass options straight through including signal!
                    .then(res => {
                    ...
                    .catch(err => {
                        // Handle AbortError gracefully to prevent uncaught promises
    ...
    async function pollForDepositSuccess(amount) {
        ...
        // ⚡ UPGRADE: Register interval to router cleanup window._tidyeIntervals
    ...
    window.processDeposit = async function() {
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                // ⚡ UPGRADE: Register interval to router cleanup
                if (!window._tidyeIntervals) window._tidyeIntervals = [];
    ...
    function isTokenStillValid() {
        return getActiveTurnstileToken() && (Date.now() - tokenCreatedAt < 90000);
    }

    async function waitForTurnstileToken() {
        let token = getActiveTurnstileToken();
        if (token) {
            turnstileToken = token;
    ...
    if (!getActiveTurnstileToken()) {
        console.log("⏳ Waiting for Turnstile to initialize via router...");
    }
...
// ── LIFECYCLE HOOKS FOR ROUTER v11 (Optional) ──
