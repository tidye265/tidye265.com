// deposit.js - SPA‑Ready Page Controller (v2)

// ── CORE FUNCTION: DEPOSIT PAGE INIT ──
// Router imayimba iyi pambuyo pa DOM settled ndi Turnstile re‑rendered.
window.initDepositPage = async function() {
    "use strict";

    console.log("⚡ [DEPOSIT JS] initDepositPage started.");

    // ── DOM REFERENCES (fresh each time) ──
    ...
    // ── FETCH FUNCTIONS (using fetchWithRetry) ──
    ...
    window.processDeposit = async function() {
        ...
        // Turnstile token – router imayamba Turnstile, koma tikufuna token kuti tigwire deposit
    ...
    // ── TURNSTILE TOKEN HELPER (router imayamba render, koma tikufuna token) ──
    ...
    function refreshTurnstile() {
        ...
        // Router imayamba re‑render, koma token yathu ikusintha.
    }

    // ── EVENT LISTENERS (attached fresh each time) ──
    ...
    // ── EXECUTE FETCH & INIT ──
    // Router imayamba Turnstile ndi cleanup, kotero ife timangofetch ndi kuyika UI.
    ...
    // Import turnstile token from session if available
    if (!importDashboardTurnstileToken()) {
        // Router idzayamba Turnstile, kotero tidikira token kuchokera ku session.
    }
...
// ── EVENT LISTENER: TRIGGER ON SPA NAVIGATION ──
window.addEventListener('spa:pageLoaded', (e) => {
    if (e.detail.page === 'deposit.html') {
        initDepositPage();
    }
});
