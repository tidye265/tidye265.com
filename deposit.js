{
    ...
    // Izi ndi zomwe zikuyamba kugwira ntchito pa navigation iliyonse (zosakhazikika)
    window.depositInit = function() {
        // Kukhazikitsa "See More" button
        setupSeeMore();

        // Kukhazikitsa Turnstile
    ...
    // ── PAGE INIT (Router imayimba iyi pa navigation iliyonse) ──
    window.pageInit = async function() {
        if (!enforceSession()) return;
        await checkApiLock();

        initBalanceFromCache();
        initNetworkFromCache();
        ...
        if (!globalUserPhone) {
            console.warn("User profile not loaded, retrying in 3 seconds...");
            setTimeout(async () => { await fetchUserProfile(); }, 3000);
        }

        await fetchNotifications();

        // Itanani depositInit kuti ikhazikitse zonse zokhudzana ndi deposit page
        if (typeof window.depositInit === 'function') {
            window.depositInit();
        }
    };

    // ── ALIAS INIT ──
    // Izi ndi zomwe router iyenera kuitana kuti page igwire ntchito bwino (await)
    window.init = async function() {
        await window.pageInit();
    };

    // ── EVENT DELEGATION ──
    // Gwiritsani ntchito Event Delegation kuti musayike event listeners nthawi iliyonse
