...
window.initDepositPage = async function() {
    ...
    // ── CACHE HELPERS (already defined in IIFE, but we redeclare for clarity) ──
    // (We'll use the same ones from IIFE, but they are already in scope)
    // Since they are in the IIFE, they are not accessible here, so we need to re-define them.
    // To avoid duplication, we can reference the global variables set in IIFE.

    // Actually, to keep it clean, we'll re-define the necessary helpers inside this function.
    // But we already have them in the IIFE. Let's just reference the global variables.
    // The IIFE sets window.global* variables, so we can use them.

    // ── PROCESS DEPOSIT ──
    // This function uses globalUserPhone, globalDetectedNetwork, etc.
    // We'll define it here, referencing window.global*.
    ...
    // The function getCleanSessionToken is defined in IIFE, we can reuse it via window
    function getCleanSessionToken() {
        let rawToken = localStorage.getItem('tidye_session_token');
        if (!rawToken || rawToken === 'null' || rawToken === 'undefined') return null;
        try {
