// deposit.js - SPA‑Ready Page Controller (v2 - Fixed Supabase Wait & JSON Parse Errors)
window.initDepositPage = async function() {
    "use strict";

    console.log("⚡ [DEPOSIT JS] initDepositPage started.");

    // ── FIX: Wait for Supabase variables to be defined ──
    async function waitForSupabase() {
        let attempts = 0;
        const maxAttempts = 20; // ~2 seconds
        while (typeof window.SUPABASE_URL === 'undefined' || typeof window.SUPABASE_ANON_KEY === 'undefined') {
            if (attempts >= maxAttempts) {
                throw new Error("Supabase variables are missing. Please configure them in your layout by adding `<script src='supabase.js'></script>` inside the `<head>` tag of your `index.html` file, **ABOVE** the `router.js` script.");
            }
            attempts++;
    ...
    // ── FIX: Added strict `if (!res.ok)` check to prevent JSON parse error on HTML ──
    async function fetchUserProfile() {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                return; // Stop processing to avoid JSON parsing error
            }
            const d = await res.json();
            cacheProfile(d);
            globalUserId = d.id || '---';
