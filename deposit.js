...
// ── AUTO-FETCH IIFE: Imfetche data nthawi yomweyo popanda refresh ──
(async function autoFetchPageData() {
    "use strict";

    console.log("🔄 Auto-fetching deposit page data in background...");

    // DOM elements – onetsani loading placeholders
    ...
    if (userIdEl) userIdEl.textContent = "Loading...";
    ...
    if (getCleanSessionToken()) {
        ...
        console.log("✅ [AUTO-FETCH] Data fetched successfully.");
    } else {
        console.warn("⚠️ No session, skipping data fetch. Placeholders remain.");
        // Set placeholders if no session
        if (balanceEl) balanceEl.textContent = "MWK 0.00";
