(function() {
    ...
    let allMatches = [], favouriteLeagues = JSON.parse(localStorage.getItem('tidye_fav_leagues') || '[]');
    ...
    // 🔥 SPORTMONKS: Odds zili kale mkati mwa get-sport-api response.
    // Palibe kufetcha kwina kuchokera ku get-odds-api.
    async function fetchMatches() {
        ...
        try {
            const res = await fetch(`${window.SUPABASE_URL}/functions/v1/get-sport-api`, {
            ...
            if (data.code === 0 && data.data) {
                ...
                buildLeagueFilter();
                buildDateFilter();
                // ✅ Render — odds zili kale mkati mwa allMatches
                renderVisible();
            } else {
    ...
    // 🔥 Gwiritsani ntchito odds zomwe zili kale mu match object
    function renderVisible() {
        ...
        visible.forEach(m => {
            ...
            // ✅ Odds zikubwera kale mkati mwa match object
            const hasOdds = m.homeOdds && m.drawOdds && m.awayOdds;
            const homeOdds = hasOdds ? m.homeOdds : null;
            const drawOdds = hasOdds ? m.drawOdds : null;
            const awayOdds = hasOdds ? m.awayOdds : null;
    ...
    window.handleOddsClick = function(matchId, selection, odds, homeTeam, awayTeam, league, event) {
        ...
        slip = slip.filter(b => String(b.matchId) !== String(matchId));
    ...
    async function fetchBackgroundPrefetch() {
        ...
        try {
            const res = await fetch(`${window.SUPABASE_URL}/functions/v1/background-prefetch`, {
                ...
                body: JSON.stringify({ page: 'matches' })
            });
            if (res.ok) {
