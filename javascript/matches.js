(function() {
    ...
    function generateDemoOdds(matchId) {
        let hash = 0;
        ...
        const rng = seededRandom(Math.abs(hash) + 12345);
        const home = 1.2 + rng() * 4.0;
        const draw = 2.5 + rng() * 2.5;
        const away = 1.5 + rng() * 5.0;
        return { home: home.toFixed(2), draw: draw.toFixed(2), away: away.toFixed(2), initHome: null, initDraw: null, initAway: null };
    }
    ...
    function renderVisible() {
        ...
        visible.forEach(m => {
            ...
            if (!odds || !odds.home || odds.home === null) {
                odds = generateDemoOdds(m.matchId);
            }
