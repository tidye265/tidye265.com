// router.js - Smart Diagnostic SPA Engine for tiDye265 (CSP Compliant)
(function() {
    'use strict';
    ...
    async function navigate(url) {
        ...
        try {
            ...
            // ── 4. EXECUTE JS WITH INLINE SCRIPT (CSP compliant) ──
            // Use a global error handler to catch runtime errors
            let executionError = null;
            const originalOnError = window.onerror;

            window.onerror = function(message, source, lineno, colno, error) {
                executionError = error || new Error(message);
            ...
            // Note: Inline scripts execute synchronously, so after append it's done.
            // We'll remove it after a short delay to ensure execution.
            setTimeout(() => {
                if (scriptElement.parentNode) scriptElement.parentNode.removeChild(scriptElement);
            }, 0);

            // Restore original onerror after a tiny delay to allow execution
            setTimeout(() => {
                window.onerror = originalOnError;
            }, 50);

            // Wait a bit for synchronous execution
            ...
            if (executionError) {
                ...
                showErrorMessage(
                    ...
                    `An error occurred while executing <strong>${scriptFile}</strong>.<br><br><code style="background: #f1f1f1; padding: 8px; display: block; border-radius: 4px;">${executionError.message}</code>`
                );
                console.groupEnd();
                return;
            }
