// router.js - Browser-Like SPA Engine (v9)
(function() {
    'use strict';
    ...
    function cleanupPage() {
        ...
        // 2. Clear intervals and timeouts (tracked globally)
        ...
        // 3. Remove page-specific event listeners (if any)
        // We rely on using a single click delegation handler (attached once)
        // but we'll remove any page-specific listeners if you use them.
        // For now, we keep the global click delegation.

        // 4. Reset page globals (init functions, fetch, etc.)
        // We do NOT delete them, but we let the new page override them.
        // However, we clear any old state variables.
        // We'll let the new init function redefine everything.

        // 5. Remove any leftover Turnstile widgets
    ...
    function injectPageScript(jsUrl) {
        return new Promise((resolve) => {
            // Remove any previous script with data-spa-script
            ...
            script.onerror = () => resolve(); // don't crash if file missing
    ...
    async function navigate(url) {
        ...
        try {
            ...
            // 7. CALL INIT FUNCTION (if defined)
            if (typeof window[initFunction] === 'function') {
                try {
                    await window[initFunction]();
                    console.log(`🎉 ${initFunction} executed successfully`);
                } catch (initErr) {
                    console.error(`❌ Error in ${initFunction}:`, initErr);
                    showErrorMessage('Initialization Error', initErr.message);
                    console.groupEnd();
                    return;
                }
            } else {
                console.warn(`⚠️ ${initFunction} not defined after loading ${scriptFile}`);
            }
    ...
    // ── INTERCEPTORS ──
    // Use a single global click handler with event delegation
