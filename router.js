// router.js - Master SPA Lifecycle & Verification Engine
(function() {
    'use strict';

    // ── PAGE CONTROLLERS MAP ──
    // Izi zimasinthidwa kuti zigwirizane ndi event-driven architecture
    ...
    function injectPageScript(jsUrl) {
        return new Promise((resolve) => {
            // Remove any previous SPA script
    ...
    async function navigate(url) {
        ...
        try {
            ...
            // 2. Get controller (to know which script to load)
            ...
            // 3. Inject script (using data-spa-script)
            await injectPageScript(scriptFile);

            // 4. DOM settled
            await new Promise(r => setTimeout(r, 0));

            // 5. AUTO RE-RENDER CLOUDFLARE TURNSTILE (if present)
            if (window.turnstile) {
                ...
                if (turnstileContainer) {
                    try {
                        ...
                        console.log("🛡️ [TURNSTILE] Re-rendered successfully on SPA route!");
                    } catch (tErr) {
            ...
            // 7. Remove loader/error messages
